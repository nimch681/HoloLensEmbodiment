...
public class ElemRenderer : MonoBehaviour
{
    ...
    public void UpdateMesh(float[] arrVertices, byte[] arrColors, int nPointsToRender, int nPointsRendered)
    {
        if (imageTarget != null)
        {
            ...
            for (int i = 0; i < nPoints; i++)
            {
                ...
                //points[i] = new Vector3(arrVertices[ptIdx + 0]+imageTarget.transform.position.x, arrVertices[ptIdx + 1]+imageTarget.transform.position.y, arrVertices[ptIdx + 2]+ imageTarget.transform.position.z);
                points[i] = new Vector3(arrVertices[ptIdx + 0], arrVertices[ptIdx + 1]+0.10f, arrVertices[ptIdx + 2]);
                indices[i] = i;
    ...
    {
        imageTarget = GameObject.FindGameObjectWithTag("Target");
